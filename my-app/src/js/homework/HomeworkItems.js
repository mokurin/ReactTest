...
export default function HomeworkItems(props) {
    ...
    const getItems = () => {
        let items = allHomeworkInfo.map(
            (info, index) => {
                return <HomeworkInfo filePaths={info.data.annex_file_paths} delHomeworkInfo={delHomeworkInfo} allHomeworkInfo={allHomeworkInfo} updateHomeworkInfo={updateHomeworkInfo} subData={subData} homeworkData={info} key={index} id={`homework${index}`} />;
            }
        )
        return items;
    }
